...
export function FinancialTransactionDialog({
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      (!Number.isSafeInteger(privateShareCents) || privateShareCents < 0 ||
        !Number.isSafeInteger(mealInputVatCents) || mealInputVatCents < 0 ||
              ...
              {selectedCategory?.code === "business_meal" ? (
                <div className="grid gap-4 rounded-2xl border border-blue-500/30 bg-blue-500/5 p-4 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Der geschäftliche Anteil wird automatisch zu 70 % als EÜR-Aufwand und zu 30 % als nicht abzugsfähig erfasst.
                    </p>
                  </div>
                  <Field>
                    <FieldLabel htmlFor="financial-private-share">Privatanteil brutto</FieldLabel>
                    <Input id="financial-private-share" type="number" min="0" step="0.01" value={privateShare} onChange={(event) => setPrivateShare(event.target.value)} />
                    <FieldDescription>Wird als Privatentnahme dokumentiert und nicht in die EÜR übernommen.</FieldDescription>
                  </Field>
                  <Field>
                    <FieldLabel htmlFor="financial-meal-input-vat">Abziehbare Vorsteuer</FieldLabel>
                    <Input id="financial-meal-input-vat" type="number" min="0" step="0.01" value={mealInputVat} onChange={(event) => setMealInputVat(event.target.value)} />
                    <FieldDescription>Nur eintragen, wenn du vorsteuerabzugsberechtigt bist und der Beleg die Umsatzsteuer ausweist.</FieldDescription>
                  </Field>
                </div>
              ) : null}
              <Field>
